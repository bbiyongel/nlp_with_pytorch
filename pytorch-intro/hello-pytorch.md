...
이 책은 자연어처리에 관련한 내용을 다루고 있습니다. 독자들의 이해를 돕고자 파이토치를 도구로 활용하여 직접 구현하는 방법에 대해서 설명하고자 합니다. 따라서 앞으로 대부분의 내용(특히 딥러닝을 활용한 부분)들은 파이토치를 활용하여 예제를 설명 할 것 입니다. 이에 앞서 간략하게나마 파이토치에 대해서 셜명하고 다루고 넘어가고자 합니다. 아쉽게도 이 책은 파이토치에 대하여 설명하는 책은 아니기 때문에 방대한 내용의 파이토치 전체를 다루기에는 할애할 수 있는 분량이 부족합니다. 다행히도 파이토치는 기존의 NumPy와 굉장히 유사한 문법을 따르고 있기 때문에, 코드를 읽고 이해하는데엔 큰 노력과 어려움이 따르진 않습니다. 좀 더 파이토치에 대해서 자세히 공부하고자 하는 독자분들은 파이토치의 잘 설명된 도큐먼트를 읽어보거나 파이토치에 대해서 다룬 책을 읽는 것을 추천 합니다.
...
이제까지 배운 것들을 활용하여 임의의 함수를 근사(approximate)하는 신경망을 구현 해 보도록 하겠습니다. -- 사실은 선형회귀분석 입니다.
