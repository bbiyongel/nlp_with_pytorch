...
time pandoc ./00-cover.md ./01-intro.md ./02-naive-bayes.md ./03-myth.md ./04-rnn.md ./05-cnn.md ./06-conclusion.md --latex-engine=xelatex -o ../${DIR_PATH}/8.text-classification.pdf --variable mainfont='Nanum Myeongjo' -V fontsize=${FONT_SIZE} -V geometry:margin=${MARGIN} -V linestretch=${LINE_SPACE}
cd ..
...
echo 'epilogue'
cd ./14-productization
time pandoc ../epilogue.md ../references.md --latex-engine=xelatex -o ../${DIR_PATH}/15.epilogue.pdf --variable mainfont='Nanum Myeongjo' -V fontsize=${FONT_SIZE} -V geometry:margin=${MARGIN} -V linestretch=${LINE_SPACE}
cd ..

time gs -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -sOutputFile=./pdf/merged.pdf ./pdf/0.preface.pdf ./pdf/1.introduction.pdf ./pdf/2.basic-math.pdf ./pdf/3.pytorch-intro.pdf ./pdf/4.preprocessing.pdf ./pdf/5.word-senses.pdf ./pdf/6.word-embedding.pdf ./pdf/7.sequential-modeling.pdf ./pdf/8.text-classification.pdf ./pdf/9.language-modeling.pdf ./pdf/10.nmt.pdf ./pdf/11.adv-nmt.pdf ./pdf/12.rl.pdf ./pdf/13.duality.pdf ./pdf/14.productization.pdf ./pdf/15.epilogue.pdf
