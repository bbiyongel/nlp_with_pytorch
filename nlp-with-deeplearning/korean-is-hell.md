...
**문제점(問題點)**이라는 단어가 **문(問, 물을 문) 제(題, 제목 제) 점(點, 점 점)**이라고 각각 segmentation 되었습니다. 그런데 **제**라는 token은 결국 embedding vector로 변환이 될 겁니다. 하지만 결제(決濟)의 **제(濟, 건널 제)**도 있고, 제공(提供)의 **제(提, 끌 제)**도 있을 겁니다. 따라서, embedding vector는 **제(題, 제목 제), 제(濟, 건널 제), 제(提, 끌 제)** 세가지 모두에 대해서 embedding을 하게 될 것이고 저 뜻의 중앙 방향으로 vector가 embedding 될 것 입니다.