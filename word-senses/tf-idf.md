...
우리는 one-hot 벡터를 통해 단어를 표현(represent)하였을 때, 많은 문제가 발생하는 것을 이야기 하였습니다. 효과적으로 정보를 추출하고 배우기 위해서는 특징(feature)를 잘 표현한 벡터가 필요 합니다. 앞으로 우리는 특징들을 잘 표현하기 위한 방법들을 딥러닝 이전의 전통적인 방식에서부터 딥러닝을 활용한 방식에 이르기까지 배우도록 하겠습니다.

이번 섹션에서는 텍스트 마이닝(Text Mining)에서 중요하게 사용되는 TF-IDF를 다뤄보겠습니다. TF-IDF를 비롯한 여러 방식들에서는 아래와 같은 가정을 합니다.

> 

TF-IDF는 이러한 관점에서 출현빈도를 사용하여 어떤 단어 $w$가 문서 $d$ 내에서 얼마나 중요한지 나타내는 수치 입니다. 이 수치가 높을 수록 $w$는 $d$를 대표하는 성격을 띄게 된다고 볼 수도 있습니다. TF-IDF의 TF는 Term Frequency로 단어의 문서 내에 출현한 횟수를 의미 합니다. 그리고 IDF는 Inverse Document Frequency로 그 단어가 출현한 문서의 숫자의 역수(inverse)를 의미 합니다. 다시 설명하면, Document Frequency (DF)는 해당 단어가 출현한 문서의 수가 되고, inverse가 붙어 역수를 취한 것 입니다.
